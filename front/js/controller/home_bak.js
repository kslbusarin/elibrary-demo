...
const global_url_api = "http://172.22.75.70/";
  ...
  if (localStorage.getItem("username")) {
    ...
    if (localStorage.getItem("uid") && localStorage.getItem("username")) {
      document.getElementById("txt_username").innerHTML =
        localStorage.getItem("uid") + " " + localStorage.getItem("username");
      document.getElementById(
        "btn_authen"
      ).innerHTML = `<a class="btn btn-sm btn-outline-light" onclick="logout()" role="button">Logout</a>`;
    } else {
      window.location.replace(global_url_api + "/test_elibrary/front/authen/");
    }
  } else {
    ...
    document.getElementById(
      "btn_authen"
    ).innerHTML = `<a class="btn btn-outline-light" href="front/authen/index.php?action=login" role="button">Login</a>`;
  }
}
    ...
    val_book += `<div class="carousel-item">
                        ...
                        <div class="card-body bg-white position-relative" style="height: 550px;">
                                ...
                                <div class="bg-white ps-2 pb-2">
                                    <div id="title${i}" class="mb-3"><b class="card-title">${sub_title(
      book_detail.title
    )}</b></div>
                                        <!-- p class="card-text">${sub_title(
                                          book_detail.authorname
                                        )}</p -->
                                        <a href=" ${global_url_api}front/book/bookdetail.php?${
      book_detail.bookid
    }" class="btn btn-sm btn-primary">อ่าน</a>
                                    </div>
                                </div>
                    ...
                    <div class="card"
                        <div class="card-body bg-white position-relative" style="height: 550px;">
                                <div class="text-center">
                                    <img src="${
                                      book_detail.coverfileurl
                                    }" id="cover_newbook"  style="width:100%;" alt="...">
                                </div>
                                <div class="position-absolute top-0 start-0"></div>
                                <div class="position-absolute top-0 end-0"></div>
                                <div class="position-absolute top-50 start-50"></div>
